...
training_file=open("hindi_train.txt")
    ...
    for line in training_lines:
        ...
        for i in word_tag1:
            if(len(i.split("_")) == 2):
                ...
                if(temp[1].find("NN")!=-1):
                    try:
                        noun_set[temp[0]].append(temp[1])
                    except:
                        noun_set[temp[0]]=[temp[1]]
                word_tag.append(i)
...
F=open("multiplelex.txt","w")
for i in noun_set:
    F.write(i)
F.close()

import os
os.system("fst-parse multiple.a multiplelex.txt > out.txt")


replace={}
f=open("out.txt")
F=open("multiplelex.txt")
l1=F.readlines()
l2=f.readlines()
print l2
for i in range(len(l1)):
    replace[l1[i]] = l2[i].split("<")

test = open("hindi_test.txt")
l3 = test.readlines()
for i in l3:
    temp=i.split(" ")
    for j in temp:
